...
func _on_entered_visible_on_screen() -> void:
	if first_time_on_screen:
		boss_hud.show_hud("Arauto da Reprovação", health_component.max_health)
...
func _on_health_component_lost_health(amount: float) -> void:
	boss_hud.update_health(health_component.current_health)
